...
#======================================================================================================================================
    ...
    #Configure each sliderflat_list
        for slid in self.sliders:
            ...
            slid.valueChanged.connect(self.sliderChanged)
...
# recieve index of slider to get value and pass to gain
    def sliderChanged(self, slider):
        self.gains=[]
        for slid in self.sliders:
            self.gains.append(slid.value())
        print(self.gains)
        #sliderValue = self.sliders[slider].value()
        self.gain(self.gains)
    
#get the signal data after gain
    def gain(self,slidersgain):
            print(self.bands[9][0])
            for i in range(10):
                self.bandsdata[i] = np.multiply(self.bands[i], slidersgain[i])
            print(self.bandsdata[9][0])
